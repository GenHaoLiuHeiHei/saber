<template>
  <div class="p-t0">
    <div class="infinite-list-wrapper" id="bookViolation">
      <ul
        class="list"
...
<script>
// 违规
import { get_violation_user_list, bookviolation_remove } from "@/api/customer/customer";
import { get_violation_book_list } from "@/api/book/library";
export default {
  name: "bookViolation",
  props: {
    formDatas: {
  ...
  computed: {
    ...
    getAjaxData () {
      return this.tofrom === 'user' ? get_violation_user_list : get_violation_book_list /* 判断是用户列表进入还是书库列表进入*/
    }
  },
  methods: {
    ...
    submitForm (formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          bookviolation_remove(this_.changeFormData.id).then(res => {
